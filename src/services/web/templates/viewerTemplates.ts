...
export interface HubServerCardInfo {
	id: string;
	name: string;
	host: string;
	botUsername: string;
...
/**
 * Render Central Viewer Hub HTML listing all running bot instances
 */
export function renderViewerHubHtml(servers: HubServerCardInfo[]): string {
	const serverCards = servers
		...
		<div class="card">
			...
			<p class="server-host">Host: <code>${s.host}</code></p>
			<p class="bot-info">Bot: <strong>${s.botUsername || "mo0nbot"}</strong></p>
			<a class="btn" href="${s.viewerUrl}">🎮 Mở 3D World Viewer</a>
		</div>`)
		.join("\n");
 ...
 */
export function renderViewerPageHtml(botInstance: Minecraft): string {
